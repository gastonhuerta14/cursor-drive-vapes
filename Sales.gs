...
function getProductInfo(payload){
  const nombre = String((payload && payload.producto) || '').trim();
  if(!nombre) throw new Error('Producto requerido');
  const ss=getSS_();
  const stock=ss.getSheetByName(SHEET_NAME);
  const H=findHeaders_(stock);
  const start=H.headerRow+1;
...
function sell(payload){
  const vendedor = String((payload && payload.vendedor) || '').trim();
  const producto = String((payload && payload.producto) || '').trim();
  const cantidadVendida = Number((payload && payload.cantidadVendida) || 1);
